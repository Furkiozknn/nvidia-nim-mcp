...
class FakeAsyncClient:
    ...
    manager. `post_side_effect` is passed straight to an AsyncMock, so it can
    be a return value, an exception, a list of either, or a callable."""
...
def fake_async_client(monkeypatch):
    ...
    `post_side_effect` before the code under test opens the client."""
    import nvidia_image

    holder = {}

    def _factory(post_side_effect=None):
        client = FakeAsyncClient(post_side_effect)
